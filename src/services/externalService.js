const getExternalOptions = async (source_id) => {
  try {
    if (source_id === 1) {
      ...
      // ✨ เพิ่ม Logic การกรอง: ถ้ามีการส่ง province_id มา ให้กรองเฉพาะอำเภอในจังหวัดนั้น
      //   if (filterId) {
      //     data = data.filter(
      //       (amphur) => amphur.province_id === parseInt(filterId)
      //     );
      //   }
